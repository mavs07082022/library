...
// Check student_dashboard.php
$dashboard = file_get_contents('student_dashboard.php');
if (strpos($dashboard, 'model_loaded') !== false) {
    echo "<p style='color:green'>✅ student_dashboard.php HAS the model_loaded fix</p>";
} else {
    echo "<p style='color:red'>❌ student_dashboard.php does NOT have the model_loaded fix</p>";
}
...
if ($config && strpos($config, 'model_loaded') !== false) {
    ...
    echo "<p style='color:red'>❌ api/config.php does NOT have the model_loaded fix or file not found</p>";
}

// Check file modification times
echo "<h2>📁 File Timestamps</h2>";
$files = ['student_dashboard.php', 'api/config.php'];
...
echo "<ul>";
echo "<li>If student_dashboard.php shows ❌, the file wasn't deployed - you need to trigger a manual deploy on HostForge</li>";
